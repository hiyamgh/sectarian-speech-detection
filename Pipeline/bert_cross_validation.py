...
def save_classification_report(y_true_categorical, y_pred_categorical, experiment_folder,accuracy, model_name='bert'):
    ...
    class_report_file_path = f'C:\\Users\\am174\\Desktop\\Twitter\\model_results\\classification_report_{model_name}.txt'
...
def eval_model(model, data_loader, device, target_list):
    ...
    val_flat_accuracy = get_acc(true_bools,pred_bools) 

    val_f1_accuracy = f1_score(true_bools,pred_bools,average='micro')*100
    #val_flat_accuracy =  accuracy_score(true_bools, pred_bools, multi_label=True) * 100
    print('F1 Validation Score : ', val_f1_accuracy)
    print('Accuracy : ', val_flat_accuracy)
...
def plot_learning_curve(train_loss_ls,val_loss_ls, experiment_name, model_name=''):
    ...
    figure_path =  f'C:\\Users\\am174\\Desktop\\Twitter\\plots\\experiments\\annotator3\\experiment_annotated\\learning_curve_{model_name}.png'
...
def run_experiment(config,folder_path):
    ...
    experiment_folder = f'model_results/{folder_path}'
    # Check if the experiment folder exists
    if not os.path.exists(experiment_folder):
       os.makedirs(experiment_folder)
    all_predictions_test, y_true_categorical,val_flat_accuracy, loss= eval_model(model, test_data_loader, device, target_list)


    save_classification_report(y_true_categorical, all_predictions_test, experiment_folder, val_flat_accuracy) 
...
def kfold_cross_validation(config, folder_path, k=10):
    model_name="bert"
    ...
    y = df_combined[target_list].values.argmax(axis=1)  # Assuming one-hot encoding of labels
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        train_data_loader = create_data_loader(df_combined.iloc[train_idx], tokenizer, config["max_length"], config["batch_size"])
        val_data_loader = create_data_loader(df_combined.iloc[val_idx], tokenizer, config["max_length"], config["batch_size"])
        ...
        optimizer = AdamW(model.parameters(), lr=config["learning_rate"], weight_decay=config["weight_decay_rate"])
        ...
        # Save classification report for the current fold
        all_predictions_test, y_true_categorical, val_flat_accuracy, loss = eval_model(model, test_data_loader, device, target_list)
        experiment_folder = f'C:\\Users\\am174\\Desktop\\Twitter\\model_results\\model_results\\{model_name}_fold{fold}'
        if not os.path.exists(experiment_folder):
            os.makedirs(experiment_folder)
        # save_classification_report(y_true_categorical, all_predictions_test, experiment_folder, val_flat_accuracy)
        report, precion_avg, hamming, j_score, macro_f2, weighted_f2,f_score = save_classification_report(y_true_categorical, all_predictions_test, experiment_folder,val_flat_accuracy,fold,  model_name)
        ...
        all_accuracy.append(val_flat_accuracy)
    ...
    class_report_file_path =f'C:\\Users\\am174\\Desktop\\Twitter\\model_results\\model_results\\mean_report.txt'
    with open(class_report_file_path, 'w') as file:
...
for annotator in annotators:
    for experiment_type in experiment_types:
        ...
        config = {
            "learning_rate": 8.663447366357565e-06,
            "batch_size": 16,
            "epochs": 3 ,
            "max_length": 100 ,
            "weight_decay_rate":3.290851114095719e-05,
        }
        kfold_cross_validation(config,current_experiment_folder)
